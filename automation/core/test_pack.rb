...
module Automation
  ...
  class TestPack < Component
    ...
    def filter_test_names(key, values, test_names = glob_test_names('*'))
      ...
      raise ExecutionError.new("Cannot apply filter '#{key}' - method '#{filter_method}' not found") unless respond_to?(filter_method, true)
    ...
    def get_test_class(test_name)
      ...
      raise ExecutionError.new("Could not load test - File '#{test_file}' does not exist") unless File.exist?(test_file)
    ...
    def load_test_pack_mode_configurations
      ...
      Mode.mode_names(application, mode).each do |name|
