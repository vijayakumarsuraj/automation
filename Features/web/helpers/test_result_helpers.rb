...
module Automation
  ...
  module TestResultHelpers
    ...
    # @param [Automation::ResultsDatabase::Test] test the test whose changes are required.
    # @return [Array<Automation::Change>] an array of change objects.
    def test_changes(run_result, test)
      ...
      changes.sort.map { |change| Automation::Change.from_value(change.value) }
    end

    # Gets a list of all changes for the specified test result.
    #
    # @param [Automation::ResultsDatabase::TestResult] test_result the test whose changes are required.
    # @return [Array<Automation::Change>] an array of change objects.
    ...
    #
    # @param [Array<Automation::ResultsDatabase::TestResult>] test_results
    # @return [Array<Automation::ResultsDatabase::TestResult>]
    def test_results_filter(test_results)
      test_results.select do |tr|
        # Non TestResult items are never filtered.
        next true unless tr.kind_of?(Automation::ResultsDatabase::TestResult)
    ...
    #
    # @param [Automation::ResultsDatabase::TestResult] test_result
    ...
    def test_result_changed_failure?(test_result)
      ...
      changed = run_result.change_events.where(test_id: test, value: Automation::Change::TestChangedFailure.value).first
    ...
    def test_result_new_success?(test_result)
      run_result = test_result.run_result
      test = test_result.test
      changed = run_result.change_events.where(test_id: test, value: Automation::Change::TestStartedPassing.value).first
    ...
    # Get a string that represents the type of this test.
    #
    # @param [Automation::ResultsDatabase::Test] test
    # @return [String] the test type.
