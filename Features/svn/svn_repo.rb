...
module Automation
  ...
  module Svn
    ...
    class SvnError < Automation::Error
    end
    ...
    class SvnRepo
      ...
      def get_log(rev_from, rev_to, options = {})
        ...
        Svn::SvnLog.parse(log_xml)
      end

      private

      # Runs an SVN command. Uses the SVN executable on the local machine.
      # Raises an SvnError if the command did not execute successfully.
      #
      ...
      def svn(command, args, options = {})
        ...
        raise SvnError.new("svn '#{command}' - #{status}\n#{message}") if status != 0
